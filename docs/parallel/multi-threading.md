...
???info "哲学家就餐问题"
    ...
    ![](img/dining_philosophers_problem.png)
...
1. 尽量细分临界区，减少持有锁的范围。
2. 最好不要在持有锁的时候调用外部函数，这些函数一般不可靠，有可能会导致死锁问题。
3. 如果要使用多个锁，务必保证**所有线程都以相同的顺序给他们上锁或解锁。**
...
活锁指的是：一些线程能够并发运行（获得 CPU 时间），但是没有一个能够进入临界区。关于活锁一个比较形象的比喻是：你和一个人狭路相逢，双方需要靠边避让才能继续走下去，结果你和他*总是*往同一个方向避让，结果就是两个人站在那再也没有向前走出一步。
