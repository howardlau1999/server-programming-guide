...
经过分析，上述代码中实现的 `threadsafe_queue` 是一个基本实现了线程安全和异常安全的队列数据结构。但是它的锁粒度过粗，临界区比较大，并发度比较低。在特定场景下我们可以允许更多的并发性，而不是限制同一时刻只有一个线程访问数据结构。要想提高并发度，就要想办法将互斥锁拆分为更细粒度的锁，也就是尽量缩小临界区。
